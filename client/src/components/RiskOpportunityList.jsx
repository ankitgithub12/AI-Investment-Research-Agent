  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
      ...
      <div className="bg-dark-card border border-dark-border rounded-xl p-6 card-hover">
        <div className="flex items-center gap-2.5 mb-4 border-b border-dark-border/50 pb-3">
        ...
        <ul className="space-y-3.5">
          {safeOpportunities.map((op, index) => (
            <li key={index} className="flex items-start gap-2.5 text-sm text-dark-text-primary leading-relaxed">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 mt-2 shrink-0"></span>
              <span>{op}</span>
            </li>
          ))}
          {safeOpportunities.length === 0 && (
            <li className="text-sm text-dark-text-muted italic">No growth opportunities analyzed.</li>
          )}
        </ul>
      </div>
      ...
      <div className="bg-dark-card border border-dark-border rounded-xl p-6 card-hover">
        ...
        <ul className="space-y-3.5">
          ...
          {safeRisks.length === 0 && (
            <li className="text-sm text-dark-text-muted italic">No immediate risks analyzed.</li>
          )}
        </ul>
      </div>
    </div>
